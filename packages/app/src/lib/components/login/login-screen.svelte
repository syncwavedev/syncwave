<script lang="ts">
    ...
    import {getRpc, getAuthManager} from '../../utils/utils';
    import Envelope from '../icons/Envelope.svelte';
    import {untrack} from 'svelte';
    import KeySolidIcon from '../icons/KeySolidIcon.svelte';
    ...
    async function onPasswordSubmit(e: Event) {
        ...
        error = undefined;
        try {
            const isSignUp = authState === 'passwordSignUp';
            ...
            if (isSignUp) {
                ...
                const signInResult = await agent.signIn({
                    email: email.trim(),
                    password,
                });

                if (signInResult.type === 'success') {
                    authManager.logIn(signInResult.token);
                } else {
                    error = signInResult.type;
                }
            } else {
                const result = await agent.signIn({
                    email: email.trim(),
                    password,
                });
    ...
    const errorMessage: Record<string, string | undefined> = {
        ...
        account_email_taken: 'That email is already in use. Please try again.',
        invalid_password: 'That password is incorrect. Please try again.',
        undefined: undefined,
    };
</script>

{#snippet initialView()}
    {#if appConfig.passwordsEnabled}
        <h1 class="mb-1 font-extrabold text-xl">Sign in to your account</h1>
        <p class="text-ink-detail mb-8">
            Enter your email and password to sign in.
        </p>
        <form onsubmit={onPasswordSubmit} class="space-y-4">
            <div>
                <label for="signin-email-input" class="sr-only">Email address</label>
                <input
            ...
            <div>
                <label for="signin-password-input" class="sr-only">Password</label>
                <input
    ...
    {:else}
        ...
        <div class="space-y-8">
            <div class="space-y-4">
                ...
                {#if appConfig.passwordsEnabled}
                    <button
                        ...
                        class="btn form-button"
                        disabled={isLoading}
                        onclick={() => (authState = 'passwordSignIn')}
            ...
            {#if appConfig.passwordsEnabled || appConfig.googleClientId}
                <hr />
            {/if}
...
{#snippet passwordSignUp()}
    <h1 class="mb-1 font-extrabold text-xl">Create your account</h1>
    <p class="text-ink-detail mb-8">Enter your details to create an account.</p>
    <form onsubmit={onPasswordSubmit} class="space-y-4">
        <div>
            <label for="signup-fullname-input" class="sr-only">Full name</label>
            <input
        ...
        <div>
            <label for="signup-email-input" class="sr-only">Email address</label
            >
            <input
        ...
        <div>
            <label for="signup-password-input" class="sr-only">Password</label>
            <input
...
<div class="flex min-h-screen flex-col bg-material-base">
    ...
    <div class="w-full max-w-2xs text-center mx-auto mt-[23vh]">
        {#if authState === 'initial'}
            {@render initialView()}
        {:else if authState === 'passwordSignIn'}
            {@render passwordSignIn()}
        {:else if authState === 'passwordSignUp'}
            {@render passwordSignUp()}
        {:else if authState === 'codeInput'}
            {@render codeInputView()}
        {/if}
