...
import {type Direction, type Orientation, mergeProps, useId} from 'bits-ui';
import {Context, useDebounce} from 'runed';
import {untrack} from 'svelte';
import {box, executeCallbacks, useRefById} from 'svelte-toolbelt';
import {on} from 'svelte/events';
import type {ReadableBoxedValues} from '../box.svelte.js';
import {clamp} from '../clamp.js';
import {addEventListener} from '../events.js';
import type {BitsPointerEvent, WithRefProps} from '../types.js';
import {useResizeObserver} from '../use-resize-observer.svelte.js';
import {useStateMachine} from '../use-state-machine.svelte.js';
    ...
    props = $derived.by(
        () =>
            ({
                id: this.opts.id.current,
                style: {
                    overflowX: this.root.scrollbarXEnabled
                        ? 'scroll'
                        : 'hidden',
                    overflowY: this.root.scrollbarYEnabled
                        ? 'scroll'
                        : 'hidden',
                },
    ...
    contentProps = $derived.by(
        () =>
            ({
                 ...
                 */
                style: {
                    minWidth: this.root.scrollbarXEnabled
                        ? 'fit-content'
                        : undefined,
                },
            }) as const
...
class ScrollAreaScrollbarScrollState {
    ...
    constructor(readonly scrollbar: ScrollAreaScrollbarState) {
        ...
        const debounceScrollend = useDebounce(
            () => this.machine.dispatch('SCROLL_END'),
            100
        );
        ...
        $effect(() => {
            ...
            const scrollDirection = this.scrollbar.isHorizontal
                ? 'scrollLeft'
                : 'scrollTop';
    ...
    props = $derived.by(
        () =>
            ({
                'data-state':
                    this.machine.state.current === 'hidden'
                        ? 'hidden'
                        : 'visible',
...
class ScrollAreaScrollbarAutoState {
    ...
    constructor(readonly scrollbar: ScrollAreaScrollbarState) {
        ...
        const handleResize = useDebounce(() => {
            ...
            if (!viewportNode) return;
            const isOverflowX =
                viewportNode.offsetWidth < viewportNode.scrollWidth;
            const isOverflowY =
                viewportNode.offsetHeight < viewportNode.scrollHeight;
            this.isVisible = this.scrollbar.isHorizontal
                ? isOverflowX
                : isOverflowY;
        }, 10);
...
class ScrollAreaScrollbarVisibleState {
    ...
    thumbRatio = $derived.by(() =>
        getThumbRatio(this.sizes.viewport, this.sizes.content)
    );
    hasThumb = $derived.by(() =>
        Boolean(this.thumbRatio > 0 && this.thumbRatio < 1)
    );
    prevTransformStyle = '';
...
class ScrollAreaScrollbarSharedState {
    ...
    constructor(readonly scrollbarState: ScrollbarAxis) {
        ...
        this.handleResize = useDebounce(
            () => this.scrollbarState.onResize(),
            10
        );
        this.handleThumbPositionChange =
            this.scrollbarState.onThumbPositionChange;
        ...
        $effect(() => {
            ...
            const unsubListener = addEventListener<'wheel'>(
                document,
                'wheel',
                handleWheel,
                {
    ...
    onpointerdown(e: BitsPointerEvent) {
        ...
        this.rect =
            this.scrollbar.opts.ref.current?.getBoundingClientRect() ?? null;
...
class ScrollAreaThumbImplState {
    ...
    props = $derived.by(
        () =>
            ({
                id: this.opts.id.current,
                'data-state': this.scrollbarState.scrollbarVis.hasThumb
                    ? 'visible'
                    : 'hidden',
                style: {
                    ...
                    transform:
                        this.scrollbarState.scrollbarVis.prevTransformStyle,
                },
...
export const ScrollAreaRootContext = new Context<ScrollAreaRootState>(
    'ScrollArea.Root'
);
export const ScrollAreaScrollbarContext = new Context<ScrollAreaScrollbarState>(
    'ScrollArea.Scrollbar'
);
export const ScrollAreaScrollbarVisibleContext =
    new Context<ScrollAreaScrollbarVisibleState>('ScrollArea.ScrollbarVisible');
export const ScrollAreaScrollbarAxisContext = new Context<ScrollbarAxis>(
    'ScrollArea.ScrollbarAxis'
);
export const ScrollAreaScrollbarSharedContext =
    new Context<ScrollAreaScrollbarSharedState>('ScrollArea.ScrollbarShared');
...
export function useScrollAreaScrollbarX(props: ScrollbarAxisStateProps) {
    return ScrollAreaScrollbarAxisContext.set(
        new ScrollAreaScrollbarXState(
            props,
            ScrollAreaScrollbarVisibleContext.get()
        )
    );
}
...
export function useScrollAreaThumb(props: ScrollAreaThumbImplStateProps) {
    return new ScrollAreaThumbImplState(
        props,
        ScrollAreaScrollbarSharedContext.get()
    );
}
