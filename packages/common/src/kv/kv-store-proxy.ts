...
export class TransactionProxy<TKey, TValue> implements Transaction<TKey, TValue> {
    constructor(
        private target: Crud<TKey, TValue>,
        private handler: KVStoreProxyHandler<TKey, TValue>
    ) {}
...
export interface KVStoreProxyHandler<TKey, TValue> {
    ...
    get?(target: Crud<TKey, TValue>, key: TKey): Promise<TValue | undefined>;
    query?(target: Crud<TKey, TValue>, condition: Condition<TKey>): Promise<Cursor<TKey, TValue>>;
    put?(target: Crud<TKey, TValue>, key: TKey, value: TValue): Promise<void>;
...
export class KVStoreProxy<TKey, TValue> implements KVStore<TKey, TValue> {
    ...
    async query(condition: Condition<TKey>): Promise<Cursor<TKey, TValue>> {
        let result: Cursor<TKey, TValue>;
        if (this.handler.query) {
            result = await this.handler.query(this.target, condition);
        } else {
            result = await this.target.query(condition);
        }

        return new CursorProxy(result, this.handler);
    }
