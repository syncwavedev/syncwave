...
export interface CrdtChangeOptions<T extends Doc<Tuple>> {
    pk: T['pk'];
    kind: 'create' | 'update';
    diff: CrdtDiff<T>;
}

export type OnCrdtChange<T extends Doc<Tuple>> = (
    options: CrdtChangeOptions<T>
) => Promise<void>;
...
export class CrdtRepo<T extends Doc<Tuple>> {
    public readonly rawRepo: CrdtRepoImpl<T>;

    constructor(options: DocStoreOptions<T>) {
        this.rawRepo = new CrdtRepoImpl<T>(options);
    }
