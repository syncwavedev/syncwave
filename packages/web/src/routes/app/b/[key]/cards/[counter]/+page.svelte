<script>
	...
	import {Crdt, log, stringifyCrdtDiff} from 'syncwave-data';

	let {data} = $props();
	let {boardKey, counter} = $derived(data);

	const card = $derived(
		observe(data.initialCard, x =>
			x.getCardView({cardId: data.initialCard.id})
		)
	);
		...
		<div class="min-h-0 flex-1">
			...
			>
				...
				<div class="px-4">
					...
					/>
					{#each card.value.comments as message}
						<UserMessage {message} />
					{/each}
				</div>
			</ScrollArea>
